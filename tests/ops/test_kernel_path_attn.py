...
from fla.ops.path_attn_poly.parallel import parallel_kernel_path_attention
...
def parallel_kernel(q, k, q2, k2, v, beta, g, scale, BT=64):
...
def naive_chunk_kernel_path_attn(q, k, q2, k2, v, beta, g, scale, BT=64):
    original_dtype = q.dtype
    HQ = q.shape[1]
    H = k.shape[1]
    k = k.unsqueeze(2).expand(-1, -1, HQ // H, -1, -1).flatten(1, 2)
...
@pytest.mark.parametrize("use_forget_gate", [True])
@pytest.mark.skipif(
...
def test_parallel(
    ...
    naive_chunk_ref = parallel_kernel(
        q,
        k,
        q2,
        k2,
    ...
    # triton_parallel_ref = parallel_kernel_path_attention(
    #     q,
    #     k,
    #     q2,
    #     k2,
    ...
    # assert_close(" o", naive_parallel_ref, naive_chunk_ref, 0.005)
    assert_close(" o", naive_parallel_ref, naive_chunk_ref, 0.005)
...
def test_parallel_varlen(
    ...
    for bos, eos in zip(cu_seqlens[:-1], cu_seqlens[1:]):
        ...
        ref[:, bos:eos] = naive_chunk_kernel_path_attn(
            q[:, bos:eos],
            k[:, bos:eos],
            v[:, bos:eos],
            w[:, bos:eos],
    ...
    ref_dw, w.grad = w.grad.clone(), None
    ref_db, beta.grad = beta.grad.clone(), None
    tri, _ = parallel_kernel_path_attention(
